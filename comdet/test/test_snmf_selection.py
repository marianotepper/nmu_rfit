...
def gaussian_matrix_generator(seed):
    np.random.seed(0)
    m = 200
    n = 50
    r = 5
    x = np.abs(np.random.randn(m, r))
    y = np.abs(np.random.randn(r, n))
    mat = np.hstack((x, np.dot(x, y)))
    noiseless_mat = np.copy(mat)
    np.random.seed(seed)
    noise = 0.1 * np.random.standard_normal(size=mat.shape)
    # noise[:, :5*r] = 0
    mat += noise
    return mat, noiseless_mat, r
...
def test(seed, snmf, matrix_generator):
    ...
    def test_selection(selection):
        ...
        cols, weights = snmf(mat, ncols=r, normalize=True, selection=selection)
        ...
        plt.figure()
        plt.subplot(131)
        plt.imshow(noiseless_mat, interpolation='none', cmap=cmap)
        plt.subplot(132)
        plt.imshow(noiseless_rec, interpolation='none', cmap=cmap)
        plt.subplot(133)
        plt.imshow(np.abs(noiseless_mat - noiseless_rec), interpolation='none', cmap=cmap)
